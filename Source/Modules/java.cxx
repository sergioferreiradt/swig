...
public:
  ...
  virtual int functionWrapper(Node *n)
  {
    ...
    // TODO : A resolução do tipo java poderá ser aqui
    if ((tm = Swig_typemap_lookup("jtype", n, "", 0)))
    {
      Printf(im_return_type, "%s", tm);
      // Printf(stdout,"&&&&&&&&&&&&&&&&& Function wrapper : %s ; Type : %s\n", Getattr(n,"sym:name"), im_return_type);
    }
    else
    {
    ...
    if (proxy_flag && wrapping_member_flag && !enum_constant_flag)
    {
      ...
      Printf(stdout, "#### 12 : Function name : %s\n", getter_setter_name);
      Printf(stdout, "#### 12 : Function type : %s\n", Getattr(n,"type"));
...
Printf(stdout, "##### 22 Java Enum code: %s\n", enum_code);
  ...
  void emitProxyClassDefAndCPPCasts(Node *n)
  {
      ...
      if (baselist)
      {
        ...
        while (base.item)
        {
          if (!(GetFlag(base.item, "feature:ignore") || Getattr(base.item, "feature:interface")))
          {
            ...
            Printf(stdout,"#### 8 - Base Class Name : %s\n",baseclassname);
            if (!c_baseclassname)
            {
              String *name = getProxyName(baseclassname);
              Printf(stdout,"#### 8 - Name : %s\n",name);
              if (name)
              {
    ...
    Printf(stdout,"#### 8 - Pure Base Class : %s\n",pure_baseclass);
    Printf(stdout,"#### 8 - Wanted Base : %s\n",wanted_base);
    ...
    if (has_outerclass)
      Printv(proxy_class_def, "static ", NIL);
    if (*Char(wanted_base))
      Printf(stdout, "Extended class %s extends %s\n", Getattr(n,"sym:name"), wanted_base);                                                                  // C++ nested classes correspond to static java classes
  ...
  * ---------------------------------------------------------------------- */
  ...
  virtual int classHandler(Node *n)
  {
    ...
    if (proxy_flag)
    {
      ...
      if (!has_outerclass)
      {
        ...
        // Printf(stdout, "   FILE generated %s\n", filen);
  ...
  void proxyClassFunctionHandler(Node *n)
  {
    ...
    bool is_interface = Getattr(parentNode(n), "feature:interface") != 0 && !static_flag && Getattr(n, "interface:owner") == 0;
    ...
    if ((tm = Swig_typemap_lookup("jstype", n, "", 0)))
    {
      Printf(stdout,"### 12 Typemap for jstype : %s\n", tm);
      ...
      Printf(stdout,"### 12 Typemap for jstype (after substitute Classname): %s ; Covariant = %s\n", tm, covariant);
      Printf(return_type, "%s", tm);
      // Printf(stdout, "   ------------------- Swig typemap : %s for %s = %s\n", tm, Getattr(n, "sym:name"), Getattr(n, "type"));
      if (covariant)
    ...
    Printf(stdout, "#### 12 return type %s ; function name %s\n", return_type, proxy_function_name);
  ...
  {
    // Printf(stdout, "      ===== Member variable : %s \n", Getattr(n, "sym:name"));
    variable_name = Getattr(n, "sym:name");
  ...
  bool substituteClassname(SwigType *pt, String *tm, bool jnidescriptor = false)
  {
    ...
    if (Strstr(tm, "$javaclassname"))
    {
      ...
      Printf(stdout, "#### 12 ; Classname type : %s\n", classnametype);
      substituteClassnameSpecialVariable(classnametype, tm, "$javaclassname", jnidescriptor);
      Printf(stdout, "#### 12 ; Type after substitute special: %s\n", tm);
  ...
  String *getUpcallJNIMethod(String *descrip)
  {
    static struct
    {
      ...
      const char *method;
        ...
        {'B', "CallStaticByteMethod"}, {'C', "CallStaticCharMethod"}, {'D', "CallStaticDoubleMethod"}, {'F', "CallStaticFloatMethod"}, {'I', "CallStaticIntMethod"}, {'J', "CallStaticLongMethod"}, {'L', "CallStaticObjectMethod"}, {'S', "CallStaticShortMethod"}, {'V', "CallStaticVoidMethod"}, {'Z', "CallStaticBooleanMethod"}, {'[', "CallStaticObjectMethod"}};
  ...
  int classDirectorMethod(Node *n, Node *parent, String *super)
  {
    ...
    for (i = 0, p = l; p; ++i)
    {
      ...
      if ((c_param_type = Getattr(p, "tmap:jni")))
      {
        ...
        if ((desc_tm = Swig_typemap_lookup("directorin", tp, "", 0)) && (jdesc = Getattr(tp, "tmap:directorin:descriptor")) && (tm = Getattr(p, "tmap:directorin")) && (cdesc = Getattr(p, "tmap:directorin:descriptor")))
        {
